<script lang="ts">
	...
	import { SortableList, UserLink } from "$lib/components";
	...
	$: titleIsValid = $formData.title.length < 21 && $formData.title.length > 0;
	...
	const addLink = async (e: SubmitEvent) => {
		const userRef = doc(db, "users", $user!.uid);
...
<main class="max-w-xl mx-auto">
	{#if $userData?.username == $page.params.username}
		...
		<form class="form-control">
			<label class="flex items-start justify-center cursor-pointer label">
				<span class="mr-6 label-text">
					<div class="tooltip group-hover:tooltip-open" data-tip="If public, the world can see your profile">
		...
		{:else}
			<button on:click={() => (showForm = true)} class="block mx-auto my-4 btn btn-outline btn-info">
				Add a Link
			</button>
		{/if}
	{/if}
